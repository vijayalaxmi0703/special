...
//#region node_modules/.nitro/vite/services/ssr/assets/routes-CxAnLqRr.js
...
var VideoScene = (0, import_react.forwardRef)(function VideoScene({ active, onComplete, onEnded, frameMaxHVh = 75, frameBottomVh = 3 }, ref) {
	...
	const formatBuffered = (video) => {
		if (!video || !video.buffered) return [];
		const ranges = [];
		for (let i = 0; i < video.buffered.length; i++) ranges.push({
	...
	const logEvent = (label, extra) => {
		...
		console.log(`[VIDEO] ${label}`, {
			...
			readyState: video ? `${video.readyState} (${READY_STATES[video.readyState] ?? "UNKNOWN"})` : void 0,
			networkState: video ? `${video.networkState} (${NETWORK_STATES[video.networkState] ?? "UNKNOWN"})` : void 0,
	...
	(0, import_react.useEffect)(() => {
		...
		const timer = setInterval(() => {
			const video = videoRef.current;
			if (!video) return;
			const lastBufferedEnd = video.buffered && video.buffered.length > 0 ? Number(video.buffered.end(video.buffered.length - 1).toFixed(3)) : null;
	...
	return (0, import_react_dom.createPortal)(/* @__PURE__ */ (0, import_jsx_runtime.jsx)(import_jsx_runtime.Fragment, { children: videoNode }), document.body);
});
/**
