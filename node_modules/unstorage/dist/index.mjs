import { destr } from "./_chunks/libs/destr.mjs";
import driver from "./drivers/memory.mjs";
import { asyncCall, deserializeRaw, serializeRaw, stringify } from "./_chunks/_utils.mjs";
import { filterKeyByBase, filterKeyByDepth, joinKeys, normalizeBaseKey, normalizeKey, prefixStorage } from "./_chunks/utils2.mjs";
function createStorage(options = {}) {
	...
	const runBatch = (items, commonOptions, cb) => {
		...
		for (const item of items) {
			...
			const options = isStringItem || !item.options ? commonOptions : {
				...commonOptions,
				...item.options
			};
...
async function restoreSnapshot(driver, snapshot, base = "") {
	base = normalizeBaseKey(base);
...
const builtinDriverDependencies = {
	...
	"capacitor-preferences": { lib: {
		name: "@capacitor/preferences",
		version: "^6 || ^7 || ^8"
	} },
	"capacitorPreferences": { lib: {
		name: "@capacitor/preferences",
		version: "^6 || ^7 || ^8"
	} },
	"db0": { database: {
	...
	"vercel-runtime-cache": { lib: {
		name: "@vercel/functions",
		version: "^2.2.12 || ^3.0.0",
		optional: true
	} },
	"vercelRuntimeCache": { lib: {
		name: "@vercel/functions",
		version: "^2.2.12 || ^3.0.0",
		optional: true
	} }
};
export { builtinDriverDependencies, builtinDrivers, createStorage, filterKeyByBase, filterKeyByDepth, joinKeys, normalizeBaseKey, normalizeKey, prefixStorage, restoreSnapshot, snapshot };
