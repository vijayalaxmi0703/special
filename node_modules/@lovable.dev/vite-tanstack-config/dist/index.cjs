...
function lovableFetchBundlePreset() {
	return {
		entry: (0, node_url.fileURLToPath)(new URL("../runtime/fetch-entry.mjs", require("url").pathToFileURL(__filename).href)),
		serveStatic: false,
		noExternals: true,
	...
	return async (env) => {
		...
		if (command === "serve") {
			...
			if (isSandbox ? hmrGateOpt !== false : !!hmrGateOpt) {
				const { hmrGatePlugin } = await Promise.resolve().then(() => require("./hmr-gate.cjs"));
		...
		if (command === "serve" && isSandbox) {
			const { devServerBridgePlugin } = await Promise.resolve().then(() => require("./dev-server-bridge.cjs"));
