...
function createErrorReportAppender(options = {}) {
	const outputPath = (0, node_path.resolve)(process.cwd(), options.outputFile ?? DEFAULT_OUTPUT_FILE);
	let initialized;
	let writeQueue = Promise.resolve();
	return (report) => {
		initialized ??= (0, node_fs_promises.mkdir)((0, node_path.dirname)(outputPath), { recursive: true }).then(() => void 0);
		const write = writeQueue.then(async () => {
			const line = `${JSON.stringify({
				...report,
				receivedAt: (/* @__PURE__ */ new Date()).toISOString()
...
//#endregion
exports.createErrorReportAppender = createErrorReportAppender;
